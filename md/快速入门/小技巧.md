...
Keyboard Shortcut: `kb(workbench.action.focusFirstEditorGroup)`, `kb(workbench.action.focusSecondEditorGroup)`, `Ctrl+3`
...
> Windows / Linux: Not bound by default. Open **Keyboard Shortcuts** (`Ctrl+K Ctrl+S`) and bind `editor.action.joinLines` to a shortcut of your choice.
...
Keyboard Shortcut: `Shift+Alt+Up` or `kb(editor.action.copyLinesDownAction)`
...
Keyboard Shortcut: `Shift+Alt+Left` or `kb(editor.action.smartSelect.grow)`
...
Keyboard Shortcut: `Alt+Up` or `kb(editor.action.moveLinesDownAction)`
...
Keyboard Shortcut: `kb(editor.fold)` and `Ctrl+Shift+]`
...
Select a symbol then type `kb(editor.action.goToDeclaration)`. Alternatively, you can use the context menu or `Ctrl+click`(`Cmd+click` on macOS).
