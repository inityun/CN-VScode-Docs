...
`kb(editor.action.addSelectionToNextFindMatch)` selects the word at the cursor, or the next occurrence of the current selection.  `Ctrl+K Ctrl+D` moves the last added cursor to next occurrence of the current selection.

`⌘D`选择光标处的词，或下一个选中词出现的位置。`⌘K` `⌘D`将添加一个光标到下一个选中词所出现的位置。

![Multi-cursor-next-word](images/editingevolved/multicursor-word.gif)

> **Tip:** You can add more cursors also with `kb(editor.action.selectHighlights)`, which will add a selection at each occurrence of the current selected text or with `Ctrl+F2`, which will add a selection at each occurrence of the current word.
>
...
Quickly shrink or expand the current selection (applies to all languages). Trigger it with `Shift+Alt+Left` and `kb(editor.action.smartSelect.grow)`
