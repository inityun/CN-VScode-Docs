...
还有一个小部件，用于在编辑 `keybindings.json` 时帮助输入键绑定规则。要启动定义键绑定小部件，请按 `kb（editor.action.defineKeybinding）`。窗口小部件监听按键并在文本框及其下面呈现序列化的 `JSON` 表示，VS Code 在您当前的键盘布局下检测到的键。键入所需的组合键后，您可以按 `Enter`，然后插入规则片段。
