...
Out of the box, `kbstyle(.)`, `kbstyle(Tab)` or `kbstyle(Enter)` are accept triggers but you can also [customize these key bindings](/md/定制化/快捷键绑定.md).

开箱即用，`kbstyle(.)`, `kbstyle(Tab)` or `kbstyle(Enter)` 是接收快捷键的，但您也可以[自定义绑定快捷键](/md/定制化/快捷键绑定.md)。
