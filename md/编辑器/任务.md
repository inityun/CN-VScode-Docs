...
>**提示:**你可以键入'任务', `Space` 和任务名称通过**快速打开**(`kb(workbench.action.quickOpen)`)运行你的任务。在这个例子中是'task echo'。

>**Tip:** You can run your task through **Quick Open** (`kb(workbench.action.quickOpen)`) by typing 'task', `Space` and the command name. In this case, 'task echo'.
...
处理 `kb(workbench.action.showCommands)` 然后键入 `Run Task` 加上 `Enter` 将显示所有可得的任务。选择一个然后按 `Enter` 将执行这个任务。

Pressing `kb(workbench.action.showCommands)` and then typing `Run Task` followed by `Enter` will list all available tasks. Selecting one and pressing `Enter` will execute the task.
