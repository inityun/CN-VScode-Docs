...
> **Tip:** To loop through errors or warnings in the current file, you can press `kb(editor.action.marker.next)` or `shift+alt+f8` which will show an inline zone detailing the problem and possible code actions (if available):
